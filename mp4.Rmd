...
In order to determine the number of unoriginal movies produced since 1895, we pulled data on all movies in the IMDb[^2] tagged with the keywords `remake`, `reboot`, or `sequel`. 
...
remakes_reboots_sequels <- db %>%
  dbGetQuery(
	...
	AND t.production_year BETWEEN 1920 AND 2018
...
These might seem like pretty big numbers when you consider how many movies you watch per year--who has time to go see 77 sequels?--but when you look at the history of these unoriginal movies, the only type currently at its peak is reboots, with five films in 2017. Remakes and sequels both reached their peak long ago: in 1941 and 1994, respectively. We have fewer sequels and remakes being made now, and if we take into consideration the fact that the movie industry has grown significantly, these unoriginal films make up a smaller percentage of the movie industry than they ever have.
...
total_movies <- db %>%
  dbGetQuery(
    "SELECT production_year, count(distinct t.id) AS number
     ...
     LEFT JOIN movie_info mi ON mi.movie_id = t.id
     LEFT JOIN movie_info mi2 on mi2.movie_id = t.id
     WHERE kind_id = 1 AND production_year BETWEEN 1940 AND 2018
       AND mi.info_type_id = 1 AND mi.info >= 80 
     GROUP BY production_year;")

ggplot(total_movies, aes(x=production_year, y=number)) + geom_area()
...
As seen above, the highest percentage of unoriginal movies occurred in the 1940s. Even in the biggest year for unoriginality, 1942, sequels, remakes, and reboots made up less than 5% of movies. 
Over the last few decades, the percentage of unoriginal movies has been in considerable decline. This year, less than 1% of all movies made were unoriginal. 

An big reason for this is the quickly increasing number of movies produced. In 1942, around 2,000 movies were released. This year, there were over 50,000. Sequel, remake and reboot production just can’t seem keep up with the ever growing number of movies.
